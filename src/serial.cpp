...
void Serial::pack(const float distance, const float angle, const uint8_t mission,
                  ...
                  const uint8_t is_get_clamp_position, const uint8_t second_target_type, const uint8_t is_get_putback_position,
                  const uint8_t target_type, const uint8_t direction)
{
    ...
    memcpy(buff + 14, &second_target_type, 1);
...
void Serial::pack(State state)
{
    ...
    memcpy(buff + 14, &state.second_target_type, 1);
...
bool Serial::read_data(struct ReceiveData &buffer){
    ...
    else
    {
        ...
        memcpy(&buffer.is_push,buff_read + 6,1);
        memcpy(&buffer.is_check_target_in_yellow,buff_read + 7,1);
